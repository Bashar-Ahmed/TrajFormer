...
from models.testmha import MultiheadAttention
from models.utils import _none_or_dtype, _canonical_mask, RMSNorm, ReLU, relu, rmsnorm

class TransformerEncoder(Module):
    
    __constants__ = ['norm']

    def __init__(self, encoder_layer, num_layers, norm=rmsnorm, enable_nested_tensor=True, mask_check=True):
        super().__init__()
...
class TransformerDecoder(Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=rmsnorm):
        super().__init__()
        torch._C._log_api_usage_once(f"torch.nn.modules.{self.__class__.__name__}")
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        if activation is relu or isinstance(activation, ReLU):
            self.activation_relu_or_gelu = 1
        elif activation is gelu or isinstance(activation, torch.nn.GELU):
