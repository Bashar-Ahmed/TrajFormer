...
class SelfAttLayer(nn.Module):
    ...
    def forward(self, x, batch_mask=None, padding_mask=None, hidden_mask=None):
        x = self.layer_X_(x)
        K = self.layer_K_(x)
        V = self.layer_V_(x)
        Q0 = self.layer_Q0_(x)
...
class CrossAttLayer(nn.Module):
    def __init__(self, time_steps=91, feature_dim=128, head_num=4, k=4):
        super().__init__()
    ...
    def forward(self, agent, rg, agent_rg_mask=None, padding_mask=None, rg_valid_mask=None): # agent -> [A,T,D] / rg -> [G,T,D]
...
class SelfAttLayer_dec(nn.Module):
    ...
    def forward(self, x, batch_mask, padding_mask=None, hidden_mask=None):
        ...
        if self.across_time:
            q = x_.reshape((-1,T,D)).permute(1,0,2)
            k,v = q.clone(),q.clone()
